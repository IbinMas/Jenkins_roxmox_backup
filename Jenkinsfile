pipeline {
    ...
    environment {
        ...
        USER_CREDENTIALS_USR = credentials('proxmox_server_usr')  // Username credential ID
        USER_CREDENTIALS_PASS = credentials('proxmox_server_pass')  // Password credential ID
    }
        ...
        stage('Backup Proxmox Configuration') {
            steps {
                script {
                    ...
                    sshpass -p '${USER_CREDENTIALS_PASS}' ssh -o StrictHostKeyChecking=no ${USER_CREDENTIALS_USR}@${PROXMOX_HOST} \\
